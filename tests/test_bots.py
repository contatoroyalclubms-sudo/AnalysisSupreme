...
def mock_motor_ia():
    ...
    motor_ia = AsyncMock()
    motor_ia.analisar_mercado = AsyncMock(return_value={
...
class TestBotArbitragem:
    ...
    async def test_detectar_arbitragem_simples(self, mock_config, mock_monitor, mock_motor_ia):
        ...
        bot.exchange_manager.get_ticker = AsyncMock(side_effect=[
            {'bid': 45100, 'ask': 45200},  # Exchange 1
            {'bid': 44900, 'ask': 45000}   # Exchange 2
        ])
    ...
    async def test_executar_caso_uso_1(self, mock_config, mock_monitor, mock_motor_ia):
        ...
        bot.get_parametros_caso_uso = Mock(return_value={
...
class TestBotGrid:
    ...
    async def test_criar_grid(self, mock_config, mock_monitor, mock_motor_ia):
        ...
        grid = await bot._criar_grid(
            "test_grid", "BTC/USDT", 44000, 46000, 10, "fixo"
        )
...
class TestBotScalping:
    ...
    async def test_detectar_spread_opportunity(self, mock_config, mock_monitor, mock_motor_ia):
        ...
        bot.exchange_manager.get_ticker = AsyncMock(return_value={
            'bid': 45000,
            'ask': 45025  # Spread de 0.055%
        })
        
        bot.exchange_manager.get_orderbook = AsyncMock(return_value={
