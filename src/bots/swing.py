...
class BotSwing(BotBase):
                ...
                if all(prices[i] >= prices[j] for j in range(i - window, i + window + 1) if j != i):
                    if prices[i] == max(prices[i - window:i + window + 1]):
                        pivots.append({
                            "index": i,
                            "price": prices[i],
                            "strength": self._calcular_forca_pivot(prices, i, window)
                        })
            else:  # low
                if all(prices[i] <= prices[j] for j in range(i - window, i + window + 1) if j != i):
        ...
        for pivot in pivots:
            ...
            for nivel in niveis:
                if abs(pivot["price"] - nivel["price"]) / nivel["price"] <= tolerance:
                    ...
                    nivel["price"] = (nivel["price"] * nivel["strength"] + 
                                    pivot["price"] * pivot["strength"]) / total_strength
    ...
    def _detectar_engulfing(self, candles: List[List]) -> Optional[Dict]:
        ...
        if (close1 < open1 and  # Candle anterior bearish
            close2 > open2 and  # Candle atual bullish
        ...
        elif (close1 > open1 and  # Candle anterior bullish
              close2 < open2 and  # Candle atual bearish
