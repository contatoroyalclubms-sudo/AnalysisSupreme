...
class WebSocketManagerPremium:
    ...
    async def _process_premium_message(self, connection: PremiumConnection, 
                                     message: str, receive_time: float):
            ...
            try:
                ...
                for exchange, connections in self.connections.items():
                    ...
                    if connections:
                        ...
                        logger.info(f"📊 {exchange}: {healthy_connections}/{len(connections)} saudáveis, "
    ...
    async def get_ultra_fast_data(self, exchange: str, data_type: str) -> Optional[Dict]:
        ...
        for key, cached in self.ultra_cache.items():
            if (key.startswith(f"{exchange}_") and 
                data_type in key and 
                current_time - cached['timestamp'] <= self.cache_ttl_ms):
                return cached['data']
