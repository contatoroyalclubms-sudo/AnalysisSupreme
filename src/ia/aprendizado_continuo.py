...
class AprendizadoContinuo:
    ...
    def calcular_reward(self, trade_data: Dict) -> float:
        ...
        try:
            ...
            reward_tempo = (
                max(0, (3600 - tempo_trade) / 3600) * 5
            )  # Bonificação por eficiência
            ...
            reward_total = (
                reward_profit + reward_volume + reward_tempo - drawdown_penalty
            )
    ...
    async def treinar_modelo(
        self, trades_or_bot_name, logs_trades_or_metricas=None, metricas=None
    ):
        """
        ...
        try:
            ...
            if self._validar_politica(nova_politica, bot_name):
                ...
                logging.info(
                    f"Modelo para {bot_name} treinado com sucesso. Melhoria: {melhoria:.2f}%"
                )
                return {
                    "sucesso": True,
                ...
                return {
                    "sucesso": False,
                    "erro": "Política rejeitada na validação",
                    "trades_processados": len(trades),
                }
    ...
    def _calcular_melhoria(self, nova_politica: Dict, bot_name: str) -> float:
        ...
        try:
            ...
            prev_ant = np.array(
                [np.dot(estado, pesos_ant) + bias_ant for estado in estados_teste]
            )

            pesos_nova = nova_politica.get("pesos", np.zeros(6))
            bias_nova = nova_politica.get("bias", 0)
            prev_nova = np.array(
                [np.dot(estado, pesos_nova) + bias_nova for estado in estados_teste]
            )
    ...
    def _atualizar_politica(self, experiencias: List[Dict]) -> Dict:
        ...
        try:
            if not experiencias:
                return {
                    "success": False,
                    "message": "Sem experiências para atualizar política",
                }
            ...
            next_states = np.array(
                [exp.get("next_state", np.zeros(6)) for exp in experiencias]
            )
