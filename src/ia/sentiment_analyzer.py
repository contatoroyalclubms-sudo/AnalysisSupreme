...
class SentimentAnalyzer:
        ...
        try:
            ...
            for i in range(5):
                noticia = {
                    ...
                    "source": self.news_sources[i % len(self.news_sources)].split("/")[
                        2
                    ],
                }
    ...
    def extrair_features(self, texto: str) -> Dict:
        ...
        try:
            ...
            score_positivo = sum(
                1 for palavra in palavras_positivas if palavra in texto_lower
            )
            score_negativo = sum(
                1 for palavra in palavras_negativas if palavra in texto_lower
            )

            features = {
                "sentiment_score": (score_positivo - score_negativo)
                / max(len(texto.split()), 1),
    ...
    def _analisar_telegram(self, symbol: str = "BTC") -> float:
        ...
        return random.uniform(-0.3, 0.7)  # nosec B311
    ...
    async def analisar_sentimento(self, symbol: str = "BTC") -> float:
        ...
        try:
            ...
            twitter_score, telegram_score, reddit_score, noticias_score = (
