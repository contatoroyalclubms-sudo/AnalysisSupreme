...
class AutoTuner:
    ...
    def _inicializar_parametros_padrao(self) -> None:
        ...
        try:
            self.parametros_otimizacao = {
                "rsi_oversold": ParametroOtimizacao(
                    "rsi_oversold", 30, 20, 40, 1, "int"
                ),
                "rsi_overbought": ParametroOtimizacao(
                    "rsi_overbought", 70, 60, 80, 1, "int"
                ),
                "macd_threshold": ParametroOtimizacao(
                    "macd_threshold", 0.001, 0.0001, 0.01, 0.0001, "float"
                ),
                "bb_extreme": ParametroOtimizacao(
                    "bb_extreme", 0.1, 0.05, 0.3, 0.05, "float"
                ),
                "volume_threshold": ParametroOtimizacao(
                    "volume_threshold", 1.5, 1.0, 3.0, 0.1, "float"
                ),
                "confidence_threshold": ParametroOtimizacao(
                    "confidence_threshold", 0.6, 0.3, 0.9, 0.1, "float"
                ),
                "risk_per_trade": ParametroOtimizacao(
                    "risk_per_trade", 0.02, 0.005, 0.05, 0.005, "float"
                ),
                "stop_loss_pct": ParametroOtimizacao(
                    "stop_loss_pct", 0.02, 0.01, 0.05, 0.005, "float"
                ),
                "take_profit_pct": ParametroOtimizacao(
                    "take_profit_pct", 0.04, 0.02, 0.10, 0.01, "float"
                ),
            }

            logging.info(
                f"Inicializados {len(self.parametros_otimizacao)} parâmetros para otimização"
            )
    ...
    async def otimizar_parametros(
        self, dados_historicos: List[Dict], numero_iteracoes: int = 100
    ) -> ResultadoOtimizacao:
        ...
        try:
            ...
            if (
                not self.melhor_configuracao
                or score_final > self.melhor_configuracao.get("score", 0)
            ):
                self.melhor_configuracao = {
            ...
            logging.info(
                f"Otimização concluída em {tempo_execucao:.2f}s - Score: {score_final:.4f}"
            )
    ...
    def _avaliar_performance(
        self, parametros: np.ndarray, nomes: List[str], dados_historicos: List[Dict]
    ) -> float:
    ...
    def _simular_trading(
        self, configuracao: Dict, dados_historicos: List[Dict]
    ) -> Dict:
        ...
        try:
            ...
            for i, dados in enumerate(dados_historicos):
                ...
                sinal = self._gerar_sinal_simulado(
                    configuracao, dados_historicos[: i + 1]
                )
                ...
                if sinal["tipo"] == "buy" and not posicao_aberta:
                    ...
                    posicao_aberta = {
                        ...
                        "stop_loss": preco
                        * (1 - configuracao.get("stop_loss_pct", 0.02)),
                        "take_profit": preco
                        * (1 + configuracao.get("take_profit_pct", 0.04)),
                    }
                    ...
                    if fechar_posicao:
                        if posicao_aberta["tipo"] == "buy":
                            profit_pct = (
                                preco_atual - posicao_aberta["preco_entrada"]
                            ) / posicao_aberta["preco_entrada"]
                        else:
                            profit_pct = (
                                posicao_aberta["preco_entrada"] - preco_atual
                            ) / posicao_aberta["preco_entrada"]
                        ...
                        trade = {
                            ...
                            "duration": dados["timestamp"]
                            - posicao_aberta["timestamp"],
                        }
            ...
            if profits:
                ...
                sharpe_ratio = (
                    media_retorno / desvio_retorno if desvio_retorno > 0 else 0
                )
            else:
                sharpe_ratio = 0
    ...
    def _gerar_sinal_simulado(self, config: Dict, dados_historicos: List[Dict]) -> Dict:
        ...
        try:
            ...
            if rsi < rsi_oversold:
                ...
                return {
                    "tipo": "sell",
                    "forca": (rsi - rsi_overbought) / (100 - rsi_overbought),
                }
            else:
                return {"tipo": "hold", "forca": 0.0}
    ...
    async def otimizacao_continua(self, intervalo_horas: int = 24) -> None:
        ...
        try:
            ...
            logging.info(
                f"Iniciando otimização contínua a cada {intervalo_horas} horas"
            )
            ...
            while self.is_running:
                try:
                    ...
                    if dados_historicos and len(dados_historicos) > 100:
                        ...
                        logging.info(
                            f"Otimização contínua concluída - Score: {resultado.score_performance:.4f}"
                        )
    ...
    def _algoritmo_genetico(
        self, fitness_function, populacao_size: int = 20, geracoes: int = 10
    ) -> Dict:
        ...
        try:
            ...
            for geracao in range(geracoes):
                ...
                logging.info(
                    f"Geração {geracao + 1}/{geracoes} - Melhor fitness: {melhor_fitness:.4f}"
                )
    ...
    def _selecao_torneio(
        ...
        """Seleção por torneio"""
        try:
            indices_torneio = np.random.choice(
                len(populacao), tamanho_torneio, replace=False
            )
            ...
            for nome, valor in individuo_mutado.items():
                if np.random.random() < taxa_mutacao:
                    ...
                    if param.tipo == "int":
                        ...
                        individuo_mutado[nome] = np.clip(
                            novo_valor, param.min_valor, param.max_valor
                        )
        ...
        try:
            ...
            for key, value in best_params.items():
                if key in self.parametros_otimizacao:
                    ...
                    novo_valor = np.clip(
                        novo_valor, param_config.min_valor, param_config.max_valor
                    )
    ...
    def otimizar_parametros_estrategia(self, estrategia: str, historico) -> Dict:
        ...
        try:
            if isinstance(historico, list):
                ...
                trades = (
                    historico.get("trades", []) if isinstance(historico, dict) else []
                )
            ...
            if estrategia == "arbitragem":
                ...
                parametros_base.update(
                    {"grid_levels": 10, "range_percent": 0.05, "profit_per_grid": 0.01}
                )
            elif estrategia == "scalping":
                parametros_base.update({"quick_profit": 0.002, "max_hold_time": 300})
            elif estrategia == "momentum":
                parametros_base.update(
                    {"momentum_threshold": 0.02, "trend_confirmation": 3}
                )
            elif estrategia == "mean_reversion":
                parametros_base.update(
                    {"deviation_threshold": 2.0, "reversion_target": 0.5}
                )
            elif estrategia == "swing":
                parametros_base.update({"swing_duration": 24, "trend_strength": 0.7})
